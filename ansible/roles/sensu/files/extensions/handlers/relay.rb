...
module Sensu::Extension
  ...
  class Endpoint
    ...
    def relay_event(data)
      if @connection.connected
        ...
        Sensu::Logger.get.debug('relay.relay_event: no connection!')
      end
    end
  ...
  class Relay < Handler
    ...
    def post_init
      @settings[:store_in_carbon].keys.each do |endpoint_name|
        ep_name = endpoint_name.intern
        ep_settings = @settings[:store_in_carbon][ep_name]
        @endpoints[ep_name] = Endpoint.new(
          ep_name,
          ep_settings['host'],
          ep_settings['port'],
          ep_settings['queue_size']
        )
      end
    end
